<div class="p-4 max-w-3xl mx-auto space-y-6 bg-gray-800 text-white">
    <h1 class="text-xl font-bold">Управление Query</h1>
    ...
    @if ($editing || $creating)
        <form wire:submit.prevent="{{ $creating ? 'store' : 'update' }}" class="space-y-4 bg-gray-700 p-4 rounded mt-4 shadow-lg">
            <div>
                <label>Title</label>
                <input type="text" wire:model="title" class="w-full rounded border p-2 mt-1 text-sm shadow-md bg-gray-600 text-white placeholder-gray-300">
                @error('title') <span class="text-red-600 text-sm">{{ $message }}</span> @enderror
            </div>

            <div>
                <label>URI</label>
                <input type="text" wire:model="uri" class="w-full rounded border p-2 mt-1 text-sm shadow-md bg-gray-600 text-white placeholder-gray-300">
                @error('uri') <span class="text-red-600 text-sm">{{ $message }}</span> @enderror
            </div>

            <div>
                <label>City</label>
                <input type="text" wire:model="city" class="w-full rounded border p-2 mt-1 text-sm shadow-md bg-gray-600 text-white placeholder-gray-300">
            </div>
