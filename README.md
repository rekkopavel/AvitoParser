...
Как это работает
Запросы: В базе данных хранится список активных запросов, которые будут использоваться для парсинга. Каждый запрос содержит информацию о том, что и где искать на сайте Avito.

Парсинг данных: Для каждого запроса извлекаются данные с сайта, после чего они парсятся и сохраняются в базе данных.

Уведомления: После того как данные успешно парсены и сохранены, система проверяет наличие новых товаров и отправляет уведомления через Telegram всем подписчикам.
